...
import com.spingular.proposal.service.ProposalService;
...
import com.spingular.proposal.service.ProposalQueryService;
...
public class ProposalResourceIT {
    ...
    private static final Long DEFAULT_PROPOSAL_VOTES = 1L;
    private static final Long UPDATED_PROPOSAL_VOTES = 2L;

    private static final Long DEFAULT_PROPOSAL_USER_VOTES = 1L;
    private static final Long UPDATED_PROPOSAL_USER_VOTES = 2L;
    ...
    public static Proposal createEntity(EntityManager em) {
        Proposal proposal = new Proposal()
            ...
            .proposalVotes(DEFAULT_PROPOSAL_VOTES)
            .proposalUserVotes(DEFAULT_PROPOSAL_USER_VOTES);
        return proposal;
    }
    /**
    ...
    public static Proposal createUpdatedEntity(EntityManager em) {
        Proposal proposal = new Proposal()
            ...
            .proposalVotes(UPDATED_PROPOSAL_VOTES)
            .proposalUserVotes(UPDATED_PROPOSAL_USER_VOTES);
        return proposal;
    }
    ...
    public void createProposal() throws Exception {
        ...
        assertThat(testProposal.getProposalVotes()).isEqualTo(DEFAULT_PROPOSAL_VOTES);
        assertThat(testProposal.getProposalUserVotes()).isEqualTo(DEFAULT_PROPOSAL_USER_VOTES);
    }
    ...
    public void getAllProposals() throws Exception {
        ...
        restProposalMockMvc.perform(get("/api/proposals?sort=id,desc"))
            ...
            .andExpect(jsonPath("$.[*].proposalVotes").value(hasItem(DEFAULT_PROPOSAL_VOTES.intValue())))
            .andExpect(jsonPath("$.[*].proposalUserVotes").value(hasItem(DEFAULT_PROPOSAL_USER_VOTES.intValue())));
    }
    ...
    public void getProposal() throws Exception {
        ...
        restProposalMockMvc.perform(get("/api/proposals/{id}", proposal.getId()))
            ...
            .andExpect(jsonPath("$.proposalVotes").value(DEFAULT_PROPOSAL_VOTES.intValue()))
            .andExpect(jsonPath("$.proposalUserVotes").value(DEFAULT_PROPOSAL_USER_VOTES.intValue()));
    }
    ...
    @Test
    @Transactional
    public void getAllProposalsByProposalVotesIsEqualToSomething() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalVotes equals to DEFAULT_PROPOSAL_VOTES
        defaultProposalShouldBeFound("proposalVotes.equals=" + DEFAULT_PROPOSAL_VOTES);

        // Get all the proposalList where proposalVotes equals to UPDATED_PROPOSAL_VOTES
        defaultProposalShouldNotBeFound("proposalVotes.equals=" + UPDATED_PROPOSAL_VOTES);
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalVotesIsInShouldWork() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalVotes in DEFAULT_PROPOSAL_VOTES or UPDATED_PROPOSAL_VOTES
        defaultProposalShouldBeFound("proposalVotes.in=" + DEFAULT_PROPOSAL_VOTES + "," + UPDATED_PROPOSAL_VOTES);

        // Get all the proposalList where proposalVotes equals to UPDATED_PROPOSAL_VOTES
        defaultProposalShouldNotBeFound("proposalVotes.in=" + UPDATED_PROPOSAL_VOTES);
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalVotesIsNullOrNotNull() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalVotes is not null
        defaultProposalShouldBeFound("proposalVotes.specified=true");

        // Get all the proposalList where proposalVotes is null
        defaultProposalShouldNotBeFound("proposalVotes.specified=false");
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalVotesIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalVotes greater than or equals to DEFAULT_PROPOSAL_VOTES
        defaultProposalShouldBeFound("proposalVotes.greaterOrEqualThan=" + DEFAULT_PROPOSAL_VOTES);

        // Get all the proposalList where proposalVotes greater than or equals to UPDATED_PROPOSAL_VOTES
        defaultProposalShouldNotBeFound("proposalVotes.greaterOrEqualThan=" + UPDATED_PROPOSAL_VOTES);
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalVotesIsLessThanSomething() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalVotes less than or equals to DEFAULT_PROPOSAL_VOTES
        defaultProposalShouldNotBeFound("proposalVotes.lessThan=" + DEFAULT_PROPOSAL_VOTES);

        // Get all the proposalList where proposalVotes less than or equals to UPDATED_PROPOSAL_VOTES
        defaultProposalShouldBeFound("proposalVotes.lessThan=" + UPDATED_PROPOSAL_VOTES);
    }


    @Test
    @Transactional
    public void getAllProposalsByProposalUserVotesIsEqualToSomething() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalUserVotes equals to DEFAULT_PROPOSAL_USER_VOTES
        defaultProposalShouldBeFound("proposalUserVotes.equals=" + DEFAULT_PROPOSAL_USER_VOTES);

        // Get all the proposalList where proposalUserVotes equals to UPDATED_PROPOSAL_USER_VOTES
        defaultProposalShouldNotBeFound("proposalUserVotes.equals=" + UPDATED_PROPOSAL_USER_VOTES);
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalUserVotesIsInShouldWork() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalUserVotes in DEFAULT_PROPOSAL_USER_VOTES or UPDATED_PROPOSAL_USER_VOTES
        defaultProposalShouldBeFound("proposalUserVotes.in=" + DEFAULT_PROPOSAL_USER_VOTES + "," + UPDATED_PROPOSAL_USER_VOTES);

        // Get all the proposalList where proposalUserVotes equals to UPDATED_PROPOSAL_USER_VOTES
        defaultProposalShouldNotBeFound("proposalUserVotes.in=" + UPDATED_PROPOSAL_USER_VOTES);
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalUserVotesIsNullOrNotNull() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalUserVotes is not null
        defaultProposalShouldBeFound("proposalUserVotes.specified=true");

        // Get all the proposalList where proposalUserVotes is null
        defaultProposalShouldNotBeFound("proposalUserVotes.specified=false");
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalUserVotesIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalUserVotes greater than or equals to DEFAULT_PROPOSAL_USER_VOTES
        defaultProposalShouldBeFound("proposalUserVotes.greaterOrEqualThan=" + DEFAULT_PROPOSAL_USER_VOTES);

        // Get all the proposalList where proposalUserVotes greater than or equals to UPDATED_PROPOSAL_USER_VOTES
        defaultProposalShouldNotBeFound("proposalUserVotes.greaterOrEqualThan=" + UPDATED_PROPOSAL_USER_VOTES);
    }

    @Test
    @Transactional
    public void getAllProposalsByProposalUserVotesIsLessThanSomething() throws Exception {
        // Initialize the database
        proposalRepository.saveAndFlush(proposal);

        // Get all the proposalList where proposalUserVotes less than or equals to DEFAULT_PROPOSAL_USER_VOTES
        defaultProposalShouldNotBeFound("proposalUserVotes.lessThan=" + DEFAULT_PROPOSAL_USER_VOTES);

        // Get all the proposalList where proposalUserVotes less than or equals to UPDATED_PROPOSAL_USER_VOTES
        defaultProposalShouldBeFound("proposalUserVotes.lessThan=" + UPDATED_PROPOSAL_USER_VOTES);
    }


    @Test
    @Transactional
    public void getAllProposalsByVoteProposalIsEqualToSomething() throws Exception {
        // Initialize the database
        VoteProposal voteProposal = VoteProposalResourceIT.createEntity(em);
        em.persist(voteProposal);
        em.flush();
        proposal.addVoteProposal(voteProposal);
        proposalRepository.saveAndFlush(proposal);
        Long voteProposalId = voteProposal.getId();

        // Get all the proposalList where voteProposal equals to voteProposalId
        defaultProposalShouldBeFound("voteProposalId.equals=" + voteProposalId);

        // Get all the proposalList where voteProposal equals to voteProposalId + 1
        defaultProposalShouldNotBeFound("voteProposalId.equals=" + (voteProposalId + 1));
    }


    @Test
    @Transactional
    public void getAllProposalsByProposalUserIsEqualToSomething() throws Exception {
        // Initialize the database
        ProposalUser proposalUser = ProposalUserResourceIT.createEntity(em);
        em.persist(proposalUser);
        em.flush();
        proposal.setProposalUser(proposalUser);
        proposalRepository.saveAndFlush(proposal);
        Long proposalUserId = proposalUser.getId();

        // Get all the proposalList where proposalUser equals to proposalUserId
        defaultProposalShouldBeFound("proposalUserId.equals=" + proposalUserId);

        // Get all the proposalList where proposalUser equals to proposalUserId + 1
        defaultProposalShouldNotBeFound("proposalUserId.equals=" + (proposalUserId + 1));
    }
    ...
    public void updateProposal() throws Exception {
        ...
        assertThat(testProposal.getProposalVotes()).isEqualTo(UPDATED_PROPOSAL_VOTES);
        assertThat(testProposal.getProposalUserVotes()).isEqualTo(UPDATED_PROPOSAL_USER_VOTES);
    }
