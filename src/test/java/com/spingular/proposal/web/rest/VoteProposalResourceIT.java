...
public class VoteProposalResourceIT {
    ...
    @Test
    @Transactional
    public void getAllVoteProposalsByProfileIsEqualToSomething() throws Exception {
        // Initialize the database
        Profile profile = ProfileResourceIT.createEntity(em);
        em.persist(profile);
        em.flush();
        voteProposal.setProfile(profile);
        voteProposalRepository.saveAndFlush(voteProposal);
        Long profileId = profile.getId();

        // Get all the voteProposalList where profile equals to profileId
        defaultVoteProposalShouldBeFound("profileId.equals=" + profileId);

        // Get all the voteProposalList where profile equals to profileId + 1
        defaultVoteProposalShouldNotBeFound("profileId.equals=" + (profileId + 1));
    }
